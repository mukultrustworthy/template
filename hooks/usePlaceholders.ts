...
export function usePlaceholders(html: string) {
  const [placeholders, setPlaceholders] = useState<ElementDataMapping[]>([]);
  const [templateHtml, setTemplateHtml] = useState<string>('');

  useEffect(() => {
    if (placeholders.length > 0) {
      const newTemplateHtml = applyPlaceholders(html, placeholders);
  ...
  const handleAddPlaceholder = (
    ...
    const newPlaceholder: ElementDataMapping = {
    ...
    setPlaceholders(prevPlaceholders => {
      const existingIndex = prevPlaceholders.findIndex(
        p => p.fieldPath.join('.') === fieldPath.join('.')
      );
    ...
    if (elementId) {
      toast.success(`Added placeholder to element: ${fieldPath.join('.')}`);
    } else {
  ...
  const handleRemovePlaceholder = (indexToRemove: number) => {
    setPlaceholders(prev => {
      ...
      toast.info(`Placeholder for "${removed.fieldPath.join('.')}" removed.`);
      return prev.filter((_, i) => i !== indexToRemove);
    });
  };

  const getPlaceholdersMap = (): Map<string, string> => {
    const placeholdersMap = new Map<string, string>();
    placeholders.forEach(p => {
      placeholdersMap.set(
        p.fieldPath.join('.'), 
        p.placeholderValue ? p.placeholderValue.value : generatePlaceholder(p.fieldPath).value
      );
    });
